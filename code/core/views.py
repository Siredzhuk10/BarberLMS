...
from core.models import Course, CourseMember, CourseLike, User
...
def index(request):
    ...
    return render(
        request,
        "home.html",
        {"featured_courses": courses, "stats": statistics},
    )
...
def allCourse(request):
    ...
    return render(
        request,
        "courses.html",
        {"courses": courses, "liked_ids": liked_ids, "enrolled_ids": enrolled_ids},
    )
...
def toggleEnroll(request, course_id):
    ...
    if enrollment:
        CourseMember.objects.filter(course_id=course_id, user_id=request.user).delete()
    else:
        CourseMember.objects.create(course_id_id=course_id, user_id=request.user, roles="std")
    return redirect(next_url)
...
def accountView(request):
    enrolled_courses = Course.objects.filter(coursemember__user_id=request.user).distinct()
    liked_courses = Course.objects.filter(likes__user=request.user).distinct()
    return render(
        request,
        "account.html",
        {
