...
def index(request):
    """Display featured courses on the home page."""
...
def user_profile(request, user_id):
    """Display user profile and their courses."""
...
def course_stats(request):
    ...
    if request.headers.get("Accept", "").startswith(
        ...
        cheapest_list = Course.objects.filter(
            price=statistics["min_price"]
        )
        expensive_list = Course.objects.filter(
            price=statistics["max_price"]
        )
        popular_list = (
        ...
        return JsonResponse({
            ...
            "expensive_courses": [
                c.name for c in expensive_list
            ],
            "popular_courses": [c.name for c in popular_list],
            "unpopular_courses": [
                c.name for c in unpopular_list
            ],
        }, safe=False)
    ...
    users_with_courses = (
        User.objects
        .filter(coursemember__isnull=False)
        .distinct()
        .count()
    )
    users_without_courses = (
        User.objects
        .filter(coursemember__isnull=True)
        .count()
    )
    ...
    context = {
        ...
        "user_stats": {
            ...
            "top_user": (
                top_user["user_id__username"]
                if top_user else None
            ),
            "users_without_enrollments": (
                list(users_without_enrollments)
            ),
        },
    }
